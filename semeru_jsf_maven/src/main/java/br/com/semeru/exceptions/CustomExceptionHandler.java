...
public class CustomExceptionHandler extends ExceptionHandlerWrapper {
	...
	// Declara o construtor que recebe uma exception do tipo
	// ExceptionHandler como parâmetro
	CustomExceptionHandler(ExceptionHandler exception) {
		this.wrapped = exception;
	}

	// Sobrescreve o método ExceptionHandler que retorna a "pilha" de
	// excessões
	@Override
	...
	// Sobrescreve o método handle que é responsável por manipular as
	// exceções do JSF
	@Override
	public void handle() throws FacesException {
		...
		while (iterator.hasNext()) {
			...
			try {
				...
				// // Aqui você poderia por exemploinstanciar as
				// classes StringWriter e PrintWriter
				StringWriter stringWriter = new StringWriter();
				PrintWriter printWriter = new PrintWriter(stringWriter);
				exception.printStackTrace(printWriter);
				// Por fim você pode converter a pilha de
				// exceções em uma String
				String message = stringWriter.toString();

				// Aqui você poderia enviar um email com a
				// StackTrace
				...
				// Coloca uma mensagem de exceção no mapa da
				// request
				...
				// Tranquiliza o usuário para que ele continue
				// usando o sistema
				...
				// Renderiza a pagina de erro e exibe as
				// mensagens
