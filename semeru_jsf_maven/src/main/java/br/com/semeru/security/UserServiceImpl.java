	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		if (username != null && username.isEmpty()) {
			...
			try {
				...
				// Use assim se você usa o Spring Security
				// 3.0.5.RELEASE
				authorities.add(new GrantedAuthorityImpl(usuario.getPermissao()));
				// Já na versão 3.1.3.RELEASE essa classe foi
				// depreciada e você deve usar como no trecho
				// abaixo
				// authorities.add(new
				// SimpleGrantedAuthority(usuario.getPermissao()));
